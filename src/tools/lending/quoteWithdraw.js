 ...
 * @returns {void}
 */
export function quoteWithdraw (server) {
  ...
  server.registerTool(
    'quoteWithdraw',
    {
      title: 'Quote Withdraw from Lending Pool',
      description: `Get a quote for withdrawing tokens from a lending pool without executing.

This tool retrieves an estimated gas fee for withdrawing tokens from a DeFi lending protocol like Aave. No transaction is broadcast. This is a read-only operation.
...
Examples:
  - Use when: "How much will it cost to withdraw 100 USDT from Aave?"
  - Use when: "What are the fees for withdrawing from the lending pool?"
  - Don't use when: User wants to execute the withdraw (use withdraw instead)
  ...
  - Returns error if token symbol is not registered`,
    ...
    async ({ chain, token, amount, to }) => {
      try {
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error quoting withdraw: ${error.message}` }]
        }
      }
    }
  )
