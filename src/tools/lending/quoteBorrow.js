 ...
 * @returns {void}
 */
export function quoteBorrow (server) {
  ...
  server.registerTool(
    'quoteBorrow',
    {
      title: 'Quote Borrow from Lending Pool',
      description: `Get a quote for borrowing tokens from a lending pool without executing.

This tool retrieves an estimated gas fee for borrowing tokens from a DeFi lending protocol like Aave. No transaction is broadcast. This is a read-only operation.
...
Examples:
  - Use when: "How much will it cost to borrow 100 USDT from Aave?"
  - Use when: "What are the fees for taking a loan?"
  - Don't use when: User wants to execute the borrow (use borrow instead)
  ...
  - Returns error if token symbol is not registered`,
    ...
    async ({ chain, token, amount, onBehalfOf }) => {
      try {
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error quoting borrow: ${error.message}` }]
        }
      }
    }
  )
