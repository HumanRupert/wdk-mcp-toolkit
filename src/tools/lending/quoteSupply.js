...
export function quoteSupply (server) {
  ...
  server.registerTool(
    'quoteSupply',
    {
      title: 'Quote Supply to Lending Pool',
      description: `Get a quote for supplying tokens to a lending pool without executing.

This tool retrieves an estimated gas fee for supplying tokens to a DeFi lending protocol like Aave. No transaction is broadcast. This is a read-only operation.
...
Examples:
  - Use when: "How much will it cost to supply 100 USDT to Aave?"
  - Use when: "What are the fees for depositing USDT into the lending pool?"
  - Don't use when: User wants to execute the supply (use supply instead)
  ...
  - Returns error if token symbol is not registered`,
    ...
    async ({ chain, token, amount, onBehalfOf }) => {
      try {
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error quoting supply: ${error.message}` }]
        }
      }
    }
  )
